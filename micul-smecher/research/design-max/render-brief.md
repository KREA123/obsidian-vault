...
This brief goes with `research/09-product-design-max-2026-09-24.md`: the numbers there are the spec and this file tells Blender how to show them. **Every image is a CGI concept.** Per the v4 README rule, it must be labelled „randare / concept (CGI)" wherever it is used.
...
6. The ember sole is **invisible from any camera ≥ 15° above the table**, unless SOUL is rocking, lifted or lying down.
...
1. **Loft body L.** Use 160 rings from z = −2 to 74, dense near 0–10 and 64–74, with 256 points per ring. For z < 8, hold the z = 8 section (it gets clipped by step 3). Each ring at height z is a closed curve in (x, y):
...
3. **Sole ellipsoid E:** (x/40)² + (y/36.9)² + ((z − 39.4)/40)² ≤ 1, at z ≥ 0. **Body = L ∩ E** (Boolean INTERSECT, Exact solver, applied after the subdivision).
   - The table clips E into the flat **land**: about Ø13.8 across (x) × 12.7 (y).
   - The intersection edge is the **sole rim**: z ≈ 1.9 at the front, 1.6 at the back and ≈ 7 on the sides. In plan it is about 45 × 24.5 mm.
   - Bevel the rim edge **R1.5** (4 segments, on the sole side).
4. **Split into two objects:** the SOLE (everything below the rim, including the bevel) and the SHELL. Put a **0.15 mm dark groove** at the split: a tube of r 0.075 sunk 0.1 mm, material `gap_dark`.
...
6. **Glass:** `glass_eye(tag, r_glass=26.0, z_edge=0.0, z_top=0.08, z_floor=-0.7, eyes=..., strength=2.6, r_screen=21.88)`.
   ...
   - Orient it so its local +z = n and local +y = (0, 0.139, 0.990). The top face sits **flush** with the table (±0.05).
...
7. **Seam and openings:**
   ...
   - **Not modelled** (invisible): the crown electrode, the IR window and the code magnets.
8. **Bottom details** (all visible only in shot 5):
   - **Land coin:** FR4 Ø13.0 × 1.0 mm, inset flush in the land, `fr4`.
   - **5 gold pads,** Ø1.8, recessed 0.15, `gold`: at (0, 0), (0, ±4.2) and (±4.2, 0).
   - **TPU ring:** OD 13.8, ID 12.2, 0.2 mm proud, `tpu_clear`.
   - **2 × Torx T5 heads,** Ø2.6, flush, at (±14, +3), projected onto E, `screw`.
   - **Engraving ring** at radius 9.5 mm around the land: „SOUL · Nº 0001 · NĂSCUT ÎN ROMÂNIA", DejaVu Sans ExtraLight caps 1.0 mm tall, as a roughness + bump mask on the sole (0.3 → 0.55 roughness, bump 0.05 mm).
...
- **True rolling** (strobe and animation only):
  - **Sideways roll φ:** rotate about the y-axis around (0, 0, 39.4) and translate x by 39.4·φ (radians, toward the lean). For |φ| < 10°, pivot about the land edge at x = ±6.9 instead.
  - **Pitch θ:** rotate about the x-axis around (0, 0, 33.3) and translate y by 33.3·θ. For |θ| < 11°, pivot about the land edge at y = ±6.4.
...
4. **Liner and socket:** SOUL's pose inside the OU: land at (0, −2, 9), then tilted **6° back** about the x-axis through the land (the top moves toward +y).
   ...
   - It is filled up to the cup's inner wall, leaving a 2 mm visible lip at the cup rim (`liner` = the sole colour). **Check:** the cup's front rim sits ≥ 3 mm below SOUL's glass bottom edge.
5. **Lights:**
...
- **Body:** the face leans back 8°. Sections as in §2.2 (the flat table becomes a gentle R150 vertical convexity: bend the table by that curvature). The base is a flat **28 × 17 land**.
- **Face:** a **domed cabochon** Ø52, with a flat back and an R200 front (sag 1.70, 0.8 mm at the edge, 0.3 × 45° polished chamfer). Centre at z 38.9, flush, 0.13 gap.
- **Details:**
  - Girdle: a 0.3 mm V-groove at the widest point.
  - Speaker: a 12 × 0.9 slot in the girdle on the +x side, at z 16.
...
| `ghost` (FUM R6) | Base #7A7A80 · **Transmission 0.85** · Roughness 0.38 · IOR 1.585 · Volume Absorption #9A9AA0, density 150 · inside: a heart-plate disc Ø46 × 1, #F2EEE7, roughness 0.3 |
| `sole_ember` | Base #D8572A · Roughness 0.30 · Coat 1.0 / 0.10 / 1.5 (satin-gloss) |
| `sole_cream` · `sole_pearl` · `sole_glow` | #FFF0C8, roughness 0.30, coat 1.0 / 0.10 · `pearl` with coat roughness 0.10 · #F2EEE7, Transmission 0.9, Roughness 0.45, IOR 1.585 |
...
| `liner` · `steel` · `cork` | = the sole material of the colourway · #C9C7C3 metallic 1 r 0.25 anisotropic 0.5 · #A8845E r 0.9 |
...
## 9. Shots (the final design: 8 required)
| # | File | Scene | Camera (T; A / E / D; lens; f) | Eyes | Must read |
|---|---|---|---|---|---|
| 1 | `soul_v5_hero.png` 1600 × 2000 (+1600 × 1200 crop) | Perlă SOUL (ember sole), upright, **yaw +8°**. OU Perlă open (lid 100°, strip at 20 %) at (+95, +120, 0), yaw −20°, soft in the background. v1 studio. | T (0, −2, 37); **+24° / +9° / 375**; 100 mm; f/8, focus on the eyes | `cream_look` | SOUL fills ~55 % of the frame height; one flank highlight; no ember visible; a helmet-headed creature at 20 px |
| 2 | `soul_v5_front_00.png` + `_15.png` 1600 × 1600 | Perlă, yaw 0, on the sweep | T (0, 0, 37); **0° / 0° and 0° / 15°** / 590; 200 mm (near-orthographic); f/16 | `soul_front` | **Nothing but body + glass.** At 0° the ember hairline at the chin is ≤ 1 mm; at 15° it is invisible (founder check #1). Also export `soul_v5_shadow20.png`: a black silhouette scaled to 20 px tall, next to the v1 Ø52 coin and the v4 pebble. |
| 3 | `soul_v5_side.png` 1600 × 1200 | Perlă, seen from +x (the speaker side) | T (0, 0, 37); **+90° / 0° / 630**; 150 mm; f/16; add a thin rim strip behind (−x) to separate the silhouette from the sweep | `soul_front` | the 8° lean and wedge (27 → 20 → 15 mm), the girdle seam on the silhouette, the 12 × 0.9 slot at z 46–58, the rocker curve with ember underneath, no loop |
...
| 5 | `soul_v5_bottom.png` 1600 × 1200 | Perlă **"plays dead"**: rotated −90° about x, settled on its back, sole facing the camera | T = the land centre after posing; **0° / +18° / 260**; 100 mm macro; f/8; add a raking key: 0.15 m disk at 10° elevation from the left, 5600 K, 0.8 W | `cream_down` (it looks toward you) | the ember sole, the Ø13 FR4 coin with 5 gold pads, the TPU ring, 2 Torx heads, the engraving ring legible |
...
| 8 | `soul_v5_family.png` 2000 × 1125 | L → R: **ONIX** (cream sole, roll −5°), **CHIHLIMBAR** (onyx sole, pitch 6° back), **PERLĂ** (ember sole, upright, 10 mm forward), **LAPIS** (pearl sole, roll +6°), **FUM** (glow sole, roll −3°). 14 mm gaps on a shallow arc (R 400). Glass counter-rotated so the eyes are level; sole slivers show on the rocked ones. | T (0, 2, 36); camera (0, −660, 110) as in v4 family; 70 mm; f/11; `studio(T, 1.6)` | smug · left · front · look · wide | a family that is *alive*; ownable colour at 3 m; no pastels |
...
1. **Front:** nothing on the face but body and glass (no ring, dots, seam or logo). The glass reads L* < 5 with one glint.
...
3. **Ember:** hidden at ≥ 15° elevation; ≤ 1 mm at 0°.
