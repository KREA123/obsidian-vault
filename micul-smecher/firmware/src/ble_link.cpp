// Adapted from https://github.com/anthropics/claude-desktop-buddy src/ble_bridge.cpp
// Copyright 2026 Anthropic, PBC. MIT License (see ../LICENSE-THIRD-PARTY.md).
// Changes: builds on Arduino-ESP32 3.x with either BLE backend (NimBLE is the
// default on the S3 there; Bluedroid is what the original targets), naming.
#include "ble_link.h"
...
static void rxPush(const uint8_t* p, size_t n) {
  for (size_t i = 0; i < n; i++) {
    ...
    if (next == rxTail) return;  // full: drop, the desktop re-sends snapshots
    rxBuf[rxHead] = p[i];
    rxHead = next;
  }
}
...
class ServerCallbacks : public BLEServerCallbacks {
  void onConnect(BLEServer*) override { onLinkUp(); }
  void onDisconnect(BLEServer*) override { onLinkDown(); }
#if defined(CONFIG_BLUEDROID_ENABLED)
  void onMtuChanged(BLEServer*, esp_ble_gatts_cb_param_t* param) override { mtu = param->mtu.mtu; }
#else
  void onMtuChanged(BLEServer*, ble_gap_conn_desc*, uint16_t m) override { mtu = m; }
#endif
};

// LE Secure Connections, passkey entry: we are DisplayOnly, the desktop
// types the 6 digits we show on the screen.
class SecCallbacks : public BLESecurityCallbacks {
  ...
  void onAuthenticationComplete(ble_gap_conn_desc* desc, int status) override {
    passkey = 0;
    secure = status == 0 && desc && desc->sec_state.encrypted;
    Serial.printf("[ble] auth %s\n", secure ? "ok" : "FAIL");
    if (!secure && server && desc) server->disconnect(desc->conn_handle);
  }
#endif
};
...
void bleInit(const char* deviceName) {
  ...
  rxChar = svc->createCharacteristic(NUS_RX_UUID, BLECharacteristic::PROPERTY_WRITE |
                                                      BLECharacteristic::PROPERTY_WRITE_NR);
  rxChar->setAccessPermissions(ESP_GATT_PERM_WRITE_ENCRYPTED);
#else
  // NimBLE: encryption requirements are characteristic flags
  txChar = svc->createCharacteristic(
      NUS_TX_UUID, BLECharacteristic::PROPERTY_NOTIFY | BLECharacteristic::PROPERTY_READ_ENC);
  ...
  BLESecurity::setPassKey(false);  // random 6 digits...
  BLESecurity::regenPassKeyOnConnect(true);  // ...new ones for every pairing
