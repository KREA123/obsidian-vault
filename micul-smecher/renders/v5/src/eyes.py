...
def render(color='#FFF0C8', gx=0.0, gy=0.0, lid_top=0.19, lid_tilt=0.16, open_=1.0,
           scale=1.0, glow=1.0, extra_glow=0.0, split=0.0):
    ...
    for side in (-1, 1):
        ...
        ey += side * split * S          # v5 'dizzy': the two eyes at opposite vertical offsets
        rx = 0.12 * scale * par * S
...
VARIANTS = {
    ...
    # --- v5 (render brief v5 §6): cream by day, amber only at night
    'soul_front': dict(color='#FFF0C8', gx=0.0, gy=-0.05, lid_top=0.19, lid_tilt=0.16),
    'soul_smug': dict(color='#FFF0C8', gx=0.95, gy=0.10, lid_top=0.30, lid_tilt=0.19),
    'soul_wide': dict(color='#FFF0C8', gx=0.0, gy=-0.35, lid_top=0.06, lid_tilt=0.12, scale=1.08),
    'soul_left': dict(color='#FFF0C8', gx=-0.6, gy=-0.10, lid_top=0.22, lid_tilt=0.16),
    'soul_closed': dict(color='#FFC96B', gx=0.0, gy=0.2, lid_top=0.0, lid_tilt=0.0, open_=0.07, glow=0.6),
    'soul_dizzy': dict(color='#FFF0C8', gx=0.0, gy=0.0, lid_top=0.40, lid_tilt=0.10, split=0.035),
}
