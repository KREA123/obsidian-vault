...
VARIANTS = {
    ...
    'cream_look': dict(color='#FFF0C8', gx=0.35, gy=-0.25),        # glance to the right & up (at the camera)
    'cream_up': dict(color='#FFF0C8', gx=0.15, gy=-1.0, lid_top=0.10),  # looking up (desk)
    'amber': dict(color='#FFC96B', lid_top=0.26),                    # night light, relaxed lids
    'ice': dict(color='#D8F0FF', gx=0.3),
    'mint': dict(color='#D2FFE6'),
    'peach': dict(color='#FFDCC8', gx=-0.3),
    'lilac': dict(color='#E6DAFF', gx=0.2, gy=-0.2),
    'gold': dict(color='#FFD76B'),
}
