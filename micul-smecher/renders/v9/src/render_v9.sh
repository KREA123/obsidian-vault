#!/usr/bin/env bash
# SOUL v9 (size M) -- CGI concept renders. Blender 4.x Cycles CPU, 4 threads, ONE render at a time, then the v5
# post.py (OIDN denoise + bloom + AgX) -> ../soul_v9_<shot>.png
# Usage:  ./render_v9.sh [shot ...]        default: hero family side hand desk typing ou_night
#         PREVIEW=1 ./render_v9.sh hero    (half resolution, few samples -> $TMP/<shot>_preview.png)
#         ./render_v9.sh alive             (5 s loop: ../soul_v9_alive.gif + .mp4; each unique eye state rendered once)
#         extra Blender args after --:  ./render_v9.sh desk -- --set dk_d=820 --samples 64
set -euo pipefail
SRC="$(cd "$(dirname "$0")" && pwd)"
V5="$(cd "$SRC/../../v5/src" && pwd)"
V6="$(cd "$SRC/../../v6/src" && pwd)"
OUT="$(cd "$SRC/.." && pwd)"
TMP="${TMP_DIR:-/tmp/soul_v9_render}"
mkdir -p "$TMP"
SHOTS=(); EXTRA=()
while [ $# -gt 0 ]; do
...
[ -f "$SRC/tex/kbd_soulos3.png" ] || { python3 "$SRC/../../v8/src/tex_v8.py" && cp "$SRC/../../v8/src/tex/kbd_soulos3.png" "$SRC/tex/"; }
[ "${PREVIEW:-0}" = 1 ] && EXTRA+=(--preview)
blend() { blender -b --factory-startup --python "$SRC/soul_v9.py" -- --tmp "$TMP" --threads 4 "$@" 2>&1 \
          | grep -E "RENDERED|Error|Traceback|File \"|v9 |v6 body|wrote" | cut -c1-240 || true; }
...
for s in "${SHOTS[@]}"; do
  ...
  if [ "$s" = alive ]; then
    ...
    ffmpeg -y -loglevel error -framerate 24 -i "$TMP/alive/png/f%04d.png" -c:v libx264 -preset slow -crf 18 \
      -pix_fmt yuv420p -movflags +faststart "$OUT/soul_v9_alive.mp4"
    ffmpeg -y -loglevel error -framerate 24 -i "$TMP/alive/png/f%04d.png" \
      ...
      -loop 0 "$OUT/soul_v9_alive.gif"
    ls -la "$OUT/soul_v9_alive.mp4" "$OUT/soul_v9_alive.gif"
    continue
  fi
  blend --shot "$s" "${EXTRA[@]}"
  if [ "${PREVIEW:-0}" = 1 ]; then
    ...
    python3 "$V5/post.py" "$TMP/$s" --out "$OUT/soul_v9_$s.png" | tail -1
  fi
done
